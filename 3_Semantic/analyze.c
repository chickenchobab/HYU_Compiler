...
{	
	switch (t->kind)
	{
		...
		case WhileStmt:
		{
			...
			if (t->child[0]->type == Integer) break;
		...
		case ReturnStmt:
		{
			...
			if (t->child[0] == NULL){
				if (currentScope->func->type == Void) break;
			}
			else{
				if (t->child[0]->type == currentScope->func->type) break;
			}
		...
		case BinOpExpr:
		{
			...
			if ((a->type != b->type) || (a->type == Void || b->type == Void))
			{
				if (t->opcode == -1) InvalidAssignmentError(t->lineno);
		...
		case CallExpr:
		{
			...
			int noparam = paramNode->flag;
			int noarg = (argNode == NULL);
			int matched = 1;
			
			if (noparam && noarg);   
			else if (noparam && !noarg && argNode->type == Void)  // when an arg is void function return value
				matched = 0;   
			else if ((noarg && !noparam) || (!noarg && noparam))    
				matched = 0;
			else {                     
				while (argNode && paramNode){ 
					if (argNode->type != paramNode->type){                      // type not matched
						matched = 0;
						break;
					}
					argNode = argNode->sibling;
					paramNode = paramNode->sibling;
					if ((argNode && !paramNode) || (!argNode && paramNode)){    // count not matched
						matched = 0;
						break;
					}
				}
